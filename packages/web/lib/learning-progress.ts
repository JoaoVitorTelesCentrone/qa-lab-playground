...
import { securityProgress } from "./security-lab";
...
export function buildLearningProgress(deliverables: ChallengeDeliverables, attempts: PeopleAttempt[], cicdSolvedIds: string[] = [], securitySolvedIds: string[] = []): LearningCompetency[] {
  ...
  const securityDone = securityProgress(securitySolvedIds);
  return [
    ...
    { id: "security", name: "Segurança", score: securityDone.percent, evidence: securityDone.solved ? `${securityDone.solved}/${securityDone.total} missão(ões) concluída(s)${security ? ` · ${security} situação(ões) People Lab relacionada(s)` : ""}` : security ? `${security} situação(ões) People Lab relacionada(s)` : "Sem missão técnica concluída", nextAction: securityDone.solved === securityDone.total ? "Revisar decisões de segurança" : "Continuar o Security Lab", href: "/lab/seguranca", available: true },
  ];
}
