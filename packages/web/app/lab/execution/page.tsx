...
export default async function Page() {
  let data: { project_id: string; projects: { title?: string; description?: string } | { title?: string; description?: string }[] | null }[] = [];
  if (isSupabaseConfigured()) {
    const supabase = await createClient();
    const { data: { user } } = await supabase.auth.getUser();
    if (user) {
      const result = await supabase.from("studio_workspaces").select("project_id,projects(title,description)").eq("user_id", user.id);
      data = result.data ?? [];
    }
  }
