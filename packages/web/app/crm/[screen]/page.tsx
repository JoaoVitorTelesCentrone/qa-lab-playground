...
export async function generateMetadata({ params }: { params: Promise<{ screen: string }> }) {
  ...
  return { title: label ? `${label} · CRM | QA Lab` : "CRM | QA Lab" };
}

export default async function CrmScreenPage({ params }: { params: Promise<{ screen: string }> }) {
  const { screen } = await params;
  if (!(app.screens ?? []).some((item) => item.id === screen)) notFound();

  const { rows, settings, signedIn } = await loadPracticeEnvironment("crm");
  return <CrmApp initial={rows} settings={settings} signedIn={signedIn} screen={screen} />;
}
