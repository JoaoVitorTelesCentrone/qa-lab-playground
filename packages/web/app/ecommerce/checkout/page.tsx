...
export default function CheckoutPage() {
  ...
  // BUG #2 — CEP inválido aceito: deveria exigir 8 dígitos, aceita qualquer valor ≥ 3 chars
  async function buscarFrete() {
    if (cep.length < 3) return;
  ...
  // BUG #3 — Cupom reutilizável: sem validação server-side, pode limpar e reaplicar
  ...
  // BUG #12 — Checkout aceita pedido com itens acima do estoque
        ...
        <div className="bg-white rounded-2xl border shadow-sm p-8 max-w-md w-full text-center space-y-4">
          ...
          <div className="flex gap-3 justify-center pt-2">
            <Link href="/ecommerce/pedidos">
              <button className="h-10 px-5 rounded-lg border border-input bg-background text-sm font-medium hover:bg-accent transition-colors">
                Ver Pedidos
              </button>
            </Link>
            <Link href="/ecommerce">
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-4xl mx-auto p-4 py-8 space-y-6">
              ...
              <div key={s.key} className="flex items-center flex-1 last:flex-none">
                <div className={`flex items-center gap-2 text-sm font-medium transition-colors ${
                  ...
                  <div className={`size-7 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${
                    isActive ? "bg-primary text-white" : isPast ? "bg-green-500 text-white" : "bg-gray-200 text-gray-500"
                  }`}>
                    {isPast ? "✓" : i + 1}
                  </div>
                ...
                {i < STEPS.length - 1 && (
                  <div className={`flex-1 h-px mx-3 ${isPast ? "bg-green-300" : "bg-gray-200"}`} />
                )}
              </div>
            );
          })}
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_300px]">
          ...
          <div className="bg-white rounded-xl border p-6">
            ...
            {step === "carrinho" && (
              <div className="space-y-4">
                <h2 className="font-semibold text-base">Revisar Carrinho</h2>
                    ...
                    {cart.map(item => (
                      <div key={item.id} className="flex items-center justify-between py-3 border-b last:border-0">
                        <div className="space-y-0.5">
                          ...
                          {/* BUG #12: aviso mas não impede finalização */}
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        type="button"
                        onClick={buscarFrete}
                        disabled={freteLoading || cep.length < 1}
                        className="h-10 px-3 rounded-lg border border-input bg-background text-sm hover:bg-accent transition-colors disabled:opacity-50 whitespace-nowrap"
                      >
                        {freteLoading ? "Buscando..." : "Calcular frete"}
                      </button>
                    </div>
                    {/* BUG #2: aceita CEP com < 8 dígitos */}
                    {frete !== null && (
                      <p className="text-xs text-green-600 flex items-center gap-1">
                        <Truck className="size-3" />
                        Frete calculado: R$ {frete.toFixed(2)}
                        {cep.length < 8 && (
                          <span className="text-amber-600 ml-1">(CEP incompleto aceito — bug!)</span>
                        )}
                      </p>
                    )}
                  </div>
                ...
                <div className="border-t pt-4 space-y-2">
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      placeholder="DESCONTO20 ou FRETE10"
                      className="flex h-10 flex-1 rounded-lg border border-input bg-background px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                    />
                    <button
                      type="button"
                  ...
                  <p className="text-xs text-gray-400">
                    Dica: limpe o campo e tente o mesmo cupom mais de uma vez.
                  </p>
                </div>
          ...
          {/* ── Resumo do pedido ──────────────────────────────────────────── */}
          <div className="bg-white rounded-xl border p-5 h-fit space-y-4">
            <div className="flex items-center gap-2">
              <Package className="size-4 text-gray-400" />
              <h3 className="font-semibold text-sm">Resumo — {totalItens} item(ns)</h3>
            </div>
