...
function makeLocalOrderReference() {
  return `QL-LOCAL-${crypto.randomUUID().slice(0, 6).toUpperCase()}`;
}
...
export function CheckoutPage() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const reference = makeLocalOrderReference();
...
export function AccountPage() {
  ...
  // Hydrate browser-only demo account after mount to keep server and client markup identical.
...
export function OperationsPage() {
  ...
  // Hydrate browser-only practice state after mount to keep server and client markup identical.
  // eslint-disable-next-line react-hooks/set-state-in-effect
